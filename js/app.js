...
window.App = (() => {
  const {icon}=UI;
  const titles={home:'Inicio',modules:'Temario',module:'Temario',lessons:'Lecciones',lesson:'Lecciones',exercises:'Ejercicios',exercise:'Ejercicios',challenges:'Retos',challenge:'Retos',quizzes:'Quiz',quiz:'Quiz',lab:'Laboratorio',predict:'Predice la salida',debug:'Depura código',cheatsheet:'Cheatsheet',progress:'Mi progreso',about:'Guía de uso',operations:'Campaña Python Ops',missions:'Misiones',profile:'Perfil e insignias',incident:'Incidente de operaciones'};
  let route='home',id='',params=new URLSearchParams(),confirmCallback=null;
  const navGroup={module:'modules',lesson:'lessons',exercise:'exercises',challenge:'challenges',quiz:'quizzes',incident:'operations'};
  function parse(){
  ...
  function render({focus=false}={}){
    ...
    let view;
    const views={home:()=>GameViews.home(),modules:()=>CourseViews.modules(),module:()=>CourseViews.moduleView(id),lessons:()=>CourseViews.lessonsIndex(params),lesson:()=>CourseViews.lessonView(id),exercises:()=>PracticeViews.exercises(params),exercise:()=>PracticeViews.exercise(id),challenges:()=>PracticeViews.challenges(params),challenge:()=>PracticeViews.challenge(id),quizzes:()=>Quiz.index(),quiz:()=>Quiz.view(id),lab:()=>Lab.render(),predict:()=>PracticeViews.predict(params),debug:()=>PracticeViews.debug(params),cheatsheet:()=>ToolViews.cheatsheet(params),progress:()=>ToolViews.progress(),about:()=>ToolViews.about(),operations:()=>GameViews.operations(),missions:()=>GameViews.missions(),profile:()=>ProfileViews.profile(),incident:()=>GameViews.incident(id)};
  ...
  const actions={
    ...
    'reset-progress':()=>ask('¿Reiniciar todo el progreso?','Se borrarán las marcas, puntuaciones, XP, niveles, estrellas, misiones, incidentes, perfil y racha de este navegador. Exporta una copia antes si necesitas conservarlos.',()=>{Progress.reset();render();UI.toast('Progreso reiniciado.');},'Sí, reiniciar progreso'),
  ...
  function click(event){
    ...
    if(action?.dataset.action.startsWith('game-')){event.preventDefault();GameUI.act(action.dataset.action,action);return;}
  ...
  async function change(event){
    ...
    try{
      ...
      ask('¿Sustituir el progreso actual?','Se importará esta copia. Se recuperan también XP, misiones, incidentes y perfil si la copia los contiene; una copia antigua obtiene XP por su progreso previo. Las actividades desconocidas se omiten. El borrador del laboratorio no cambiará.',()=>{try{Progress.importData(text);render();UI.toast('Copia importada correctamente.');}catch(error){UI.toast('No se pudo importar: '+error.message);}},'Importar copia');
  ...
  function init(){
    ...
    GameUI.init();Search.init();themeLabel();closeMenu();render();
