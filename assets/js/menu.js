import { menuList, routeMap } from "./routeConfig.js";
...
export function initMenu() {
  ...
  menuRoot.innerHTML = filterMenu().map(buildItem).join("");
  loadSide();
...
function filterMenu() {
  return menuList.map((menuItem) => {
    if (menuItem.menuRoute) return canRoute(routeMap[menuItem.menuRoute]) ? menuItem : null;
    const menuItems = menuItem.menuItems.filter((itemData) => canRoute(routeMap[itemData.itemRoute]));
    return menuItems.length ? { ...menuItem, menuItems } : null;
  }).filter(Boolean);
}
